...
print (sys.argv)
file_list = GetFileList(sys.argv[1],'.lz4')
print (file_list)
flog = open('compared.log','w')
stat = open('compress.stat','w')	
dir_path = 	sys.argv[1] + '/'		
for file_name in file_list:
	lz4_name	= dir_path + file_name + '_hw.lz4'
	shuff_name	= dir_path + file_name + '_hw.shuff'
	decomp_name = dir_path + file_name + '_hw.decomp'
	src_size	= os.path.getsize(lz4_name)
	dest_size	= os.path.getsize(shuff_name)
	if filecmp.cmp(lz4_name,decomp_name) :
		flog.write("File %s\t and %s\t is the same\n" % (lz4_name,decomp_name)) 	# python3.x # 
		stat.write("%s\t=\t%d\t%s\t=\t%d\tRC=%0.4f\n" % (lz4_name,src_size,shuff_name,dest_size,float(dest_size)/float(src_size))) # python3.x # 
